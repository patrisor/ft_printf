...
/*   Updated: 2019/10/08 02:42:52 by patrisor         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int					pf_handle_string(t_pf_param param, va_list list)
{
	...
	count = 0;
	// Set up string to be stored in va_list
	str = va_arg(list, char *);
	if (str == NULL)
		str = "(null)";
	len = pf_strlen(str, INT_MAX);
	// check parameter and reset data
...
// Prints out Bytes, accounting for flags
