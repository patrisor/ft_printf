/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   format.c                                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: pbondoer <pbondoer@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/02/05 03:35:56 by pbondoer          #+#    #+#             */
/*   Updated: 2019/10/07 22:19:21 by patrisor         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"
#include <stdio.h>

//
int					get_argument(const char *str, size_t len, int is_valid,
							va_list list)
{
	// checks if currently on the '%' byte
	if (*str == '%' && is_valid)
		return (pf_transform(pf_get_param(str, len), list));
	else
		// Otherwise, write any extenuating trailing byte
		return (pf_write(str, len));
}

/*
 * Scans valid byte in the flag string
 */
static int			parse_inner(const char *str, size_t *i, int *valid)
{
	if (!str[*i])
		return (1);
	// Continues iterting through byte stream until conversion flag is found
	// pf_isvalid: If there are any flags found in the parsing, it will return a 1.
	// pf_is_conversion: Check for conversion flag is present
	while (str[*i] && pf_is_valid(str[*i]) && !pf_is_conversion(str[*i]))
		(*i)++;
	// If conversion flag is found return, 0
	if ((*valid = str[*i] && pf_is_valid(str[*i])))
		(*i)++;
	else
		return (1);
	return (0);
}

/*
 * Traverse through first parameter of printf. If a percent variable was found, then
 * scan next byte(s). If byte is invalid, then keep iterating
 * Parse a format string and print stuff
 */
int					pf_parse_format(const char *str, va_list list)
{
	size_t			i;
	size_t			start;
	int				valid;
	int				count;

	i = 0;
	count = 0;
	while (str[i])
	{
		start = i;
		valid = 0;
		// Each format specification is introduced by the percent character (``%'')
		if (str[i] == '%')
		{
			i++;
			// Scans if valid conversion flag byte is contained in the string, if not
			// the string continue over to find the valid '%' byte
			if (parse_inner(str, &i, &valid))
				continue;
		}
		else
			// Continue searching for byte
			while (str[i] && str[i] != '%')
				i++;
		// TODO:
		count += get_argument(str + start, i - start, valid, list);
	}
	return (count);
}

/*
** Creates a printf param
** NOTE: All params are 0 at the start so no init is nescesary
*/

inline t_pf_param	pf_param(const char *str, size_t len)
{
	t_pf_param	param;

	param = (t_pf_param) {
		.str = (t_pf_string){ .str = str, .length = len },
			.access = 0,
			.flags = 0,
			.width = 0,
			.width_access = 0,
			.precision = 0,
			.modifier = NONE,
			.conversion = 0,
			.error = 0
	};
	return (param);
}

inline t_pf_string	pf_string(const char *str, size_t len)
{
	return ((t_pf_string) {
			.str = str,
			.length = len
			});
}
